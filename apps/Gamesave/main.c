...
char* read_save_file(const char* name, size_t size) {
  ...
  if (extapp_fileExists(save_name, EXTAPP_FLASH_FILE_SYSTEM)){//EXTAPP_RAM_FILE_SYSTEM)) {
    size_t file_len = 0;
    const char* save_content = extapp_fileRead(save_name, &file_len, EXTAPP_FLASH_FILE_SYSTEM);// EXTAPP_RAM_FILE_SYSTEM);
    int error = LZ4_decompress_safe(save_content, output, file_len, size);
    
    // Handling corrupted save.
    if (error <= 0) {
      memset(output, 0xFF, size);
      extapp_fileErase(save_name,EXTAPP_FLASH_FILE_SYSTEM);//, EXTAPP_RAM_FILE_SYSTEM);
...
void write_save_file(const char* name, char* data, size_t size) {
  ...
  if (compressed_size > 0) {
    if (extapp_fileWrite(save_name, output, compressed_size,EXTAPP_FLASH_FILE_SYSTEM)){//, EXTAPP_RAM_FILE_SYSTEM)) {
