...
static void drawRomList(char **filenames, int nb, int selected_rom) {
  ...
  for(int i = 0; i < nb; i++) {
    ...
    strncpy(name_buffer, filenames[i], FILENAME_LENGHT_MAX);
    if(extapp_fileExists(osd_newextension(name_buffer, ".gbs"), EXTAPP_FLASH_FILE_SYSTEM)){//EXTAPP_RAM_FILE_SYSTEM)) {
  ...
  if(nb == 0) {
    ...
    for(;;) {
      ...
      if(scancode & SCANCODE_Down) {
        ...
        extapp_fileErase(osd_newextension(name_buffer, ".gbs"), EXTAPP_FLASH_FILE_SYSTEM);// EXTAPP_RAM_FILE_SYSTEM);
        drawRomList(filenames, nb, selected_rom);
        waitForKeyReleased();
