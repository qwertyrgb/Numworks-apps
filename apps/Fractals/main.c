...
//TODO: is simulated complex faster than complex? ][naming vars zreal and zimag
//TODO: Chaneg colors
    ...
    if(Zoom<65535){
      for(int x=0;x<320;x++){
        for(int y=0;y<241;y++){
          ...
          float complex c=(3.5*x/319)/Zoom+xpos+((-2.5*y/241)/Zoom+ypos)*I;
          int i=0;
          while(i<N_iteration && abs(z)<2){
            ...
            if(type==1){
              if(Power==2){
                ...
                z=cabs(creal(z))+cabs(cimag(z))*I;
                float complex tmp=z;
                for (int pw=1; pw<Power;pw++) tmp*=z;
                z=tmp;
              }
            } else{
              if(Power==2) z*=z;
